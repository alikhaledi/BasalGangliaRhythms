...
ck_1(1,:) = [1 logspace(-1,log10(4),30)]; % This is HD
...
for CON = 1:2
    ...
    parfor i = 1:size(ck_1,2)
        ...
        [r2mean,pnew,feat_sim,dum1,xsim_gl] = computeSimData_160620(R,m,uc,Pbase,0,1);
        feat{i} = feat_sim;
        xsim{i} = xsim_gl;
        
        disp([CON i])
    end
    rootan = [Rorg.path.rootn 'data\ConnectionSweep'];
    mkdir(rootan)
