...
phiList = 1:12;
cmatStim = []; smatStim = [];
for phi = 1:size(phiList,2)
    stm = 2;
    cmatStim(:,:,phi) = [abs(connectMat{1,phi,stm}+connectMat{2,phi,stm}) angle(connectMat{1,phi,stm}+connectMat{2,phi,stm})];
    smatStim(:,:,phi) = specMat{1,phi,stm};
    omatStim(:,:,phi) = [statBurstOverl{1,phi,stm}(:,:,1)+statBurstOverl{2,phi,stm}(:,:,1)];
end
...
%         [h p] = corr(A{1}(:),B{1}(:),'type','Spearman');
        ...
        clist(stateStim,stateSpont) =  mean([nmlist(stateStim,stateSpont)  slist(stateStim,stateSpont)]); %collectR2(A,B);
        %         olist(st
    end
