function plotStimTraces2(twin,m2Env,stnEnv,sw_twin,sw_PLV,dPhi,befdPhi,middPhi,aftdPhi)
...
for band = 1:2
    for phi = 1:3
        ...
        %         p(phi) = polarhistogram(circ_mean(middPhi{band,pheff,stm},[],1),-pi:pi/32:pi,'Normalization','probability');hold on;
        %         p(phi).FaceAlpha = 0.6; p(phi).EdgeAlpha = 0; p(phi).FaceColor = cmap(phi,:);
...
%% SCRIPT GRAVE
% Script for radar plot with intermediate burst angles       
%         magvec = [0.05 0.1 0.15] + ((phi-1).*0.01);
%         for i = 1:3
%             switch i
%                 case 1
%                     X = befdPhi{band,1,1}; Y = befdPhi{band,pheff,stm};
%                 case 2
%                     X = middPhi{band,1,1}; X2 = befdPhi{band,pheff,stm}; Y = middPhi{band,pheff,stm};
%                 case 3
%                     X = aftdPhi{band,1,1}; X2 = middPhi{band,pheff,stm}; Y = aftdPhi{band,pheff,stm};
%             end
%             PS =circ_mean(Y,[],1);
%             p(i) = plotPolarStem(PS,cmap(phi,:)*1,magvec(i),'-','o');
%             if phi>1
%                 stattab(:,band,phi) = radarStat(X,Y,cmap(phi,:)*1,magvec(i),pi,'*',0.05);
%                 if i>1
%                     stattab2(:,band,phi) = radarStat(X2,Y,cmap(phi,:)*1,magvec(i),pi+0.2,'sqr',0.05);
%                 end
%             end
%         end
%         a = gca;
